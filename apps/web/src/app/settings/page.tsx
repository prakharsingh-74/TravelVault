...
export default function SettingsPage() {
  ...
  const checkHealth = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.gemini_api_key) {
        setAiStatus('Active (Using custom key)');
      } else {
        setAiStatus('Active (Mock/Fallback mode - please configure key)');
      }
    } catch (e) {
      ...
      <div className="glass-panel p-6 rounded-2xl space-y-4">
        ...
        <form onSubmit={handleSaveKey} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="px-4 py-2 rounded-lg text-sm font-semibold bg-gradient-to-r from-[#8b5cf6] to-[#3b82f6] hover:opacity-90 disabled:opacity-50 text-white transition-opacity"
          >
            {isSaving ? 'Saving Key...' : 'Save Settings'}
          </button>
        </form>
      </div>
